...
- **`fishy/storage.py`** — long/tidy CSV persistence, **one CSV per tank** at `data/<tank_id>/readings.csv`. `readings_path_for(data_dir, tank_id)` is the single seam mapping a tank to its file; the low-level primitives (`load_readings(path)`, `append_reading/append_readings(path)`, `ensure_file`) each operate on ONE resolved CSV file. `Reading(tank,parameter,date,value,unit,note)`, `load_readings(path) -> LoadResult(.readings/.warnings/.first_run)`. `delete_tank_data(tank_id, data_dir)` removes a tank's whole directory (the one place data is deleted). Value formatter `_format_value` (compact `8` not `8.0`) is reused everywhere numbers render. Columns: `tank,parameter,date,value,unit,note`. `DEFAULT_DATA_DIR = data/`; per-tank dir + file created lazily on first append.
...
- **`fishy/tank_store.py`** — write-side companion to `config.py` for the mutable `[[tanks]]` array. `add_tank(config_path, id, label)` / `delete_tank(config_path, id)` do **surgical, comment-preserving text edits** to `config/fishy.toml` (stdlib `tomllib` is read-only; no TOML-writer dep). Delete also removes dangling `[overrides.<id>.*]` tables. `slugify` / `is_valid_id` / `tank_ids`. Ids must match `^[a-z0-9][a-z0-9-]*$` (URL + CSV safe, and used as the tank's data-dir name). Pairs with `storage.delete_tank_data(tank_id, data_dir)` — deletes the tank's `data/<id>/` directory.

Flask layer — `fishy/__init__.py`:

- **Config/storage wiring**: `create_app()` puts `Config` on `app.config["FISHY_CONFIG"]`; the readings **data directory** at `app.config["FISHY_DATA_DIR"]` (readings live at `<FISHY_DATA_DIR>/<tank_id>/readings.csv`, resolved via `_readings_path_for(tank_id)`). Tests inject both to use tmp data.
- **Routes**: `index` (`/` shelf), `create_tank` (POST `/tanks`, PRG → new tank), `delete_tank` (POST `/tank/<tank_id>/delete`, PRG → shelf; also purges readings), `tank_view` (`/tank/<tank_id>`), `parameter_view` (`/tank/<tank_id>/parameter/<param_id>`, GET) + `add_reading` (POST `.../reading`, PRG pattern), `aggregate_view` (`/tank/<tank_id>/aggregate`), `health`. Active tank = URL path param. `FISHY_CONFIG_PATH` on `app.config` is the injectable TOML path used to reload `FISHY_CONFIG` after a create/delete (via `_reload_config()`).
